...
GETHST,
	...
	UDATE = SDATE
	LOKCTL = 1
	XCALL ISIO (8,INVHST,UKEY1,READ,LOKCTL)
HFORWD,

	IF (LOKCTL.EQ.1) GOTO DASHES
	IF (UITMNO.NE.IITMNO) GOTO DASHES
	IF (UDATE.LT.SDATE) GOTO NXTONE
	IF (UDATE.GT.EDATE) GOTO DASHES

	FSTFLG = 1
	HLDATE(1,4) = UDATE(3,6)
	HLDATE(5,6) = UDATE(1,2)
	PLINE(1,15) = UITMNO
	PLINE(19,26) = HLDATE,DTMASK
