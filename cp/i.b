...
RECORD	VARS
	gm_eof	,d1	;hit eof on customer email address's
	email	,a80
	ekey	,a9
	tblrfa	,a6
	xpol1	,a12
...
FSELEC,
	...
	XCALL W_DISP(WND_1,WD_POS, 2,10,'COMPANY CODE             <CURRENT COMPANY>')
	...
	xcall winpt (w_id, 2,30,03,00,'AE',entry,inxctl)
;;;	goto (fselec, abort),inxctl
	...
	xcall w_disp (wnd_1,wd_pos,2,30,cmpcod)
	xcall w_updt
...
DISPLA,
	...
	USING FSELEC SELECT
;;;	USING FYEAR SELECT
	(1),	XCALL W_DISP(WND_1,WD_POS,1,1,'CURRENT ORDER FILES')
	(2),	XCALL W_DISP(WND_1,WD_POS,1,1,'CURRENT YEAR HISTORY FILES')
...
;;;
STP2,		;SHIP TO SCAN
	clear kshipto, kitem	;5-3-10 in case 2nd past thru this logic...
...
ASK6B,
	CALL CINPUT
	GOTO (DISPLA,RESELEC,screen,ASK6C), INXCTL
...
ASK7B,
	CALL CINPUT
	GOTO (DISPLA,RESELEC,screen,ASK7C), INXCTL
...
; Ship-to scan, match ship-to...
	  ...
	  DO BEGIN
	     READS (24,ORDHDR,FSCRN)
	     XCALL INSTR (1,OSHPNM(1,120),KSHIPTO(1,POS),FOUND)
	     IF (OEFLAG.NE.' ' .AND. OLOC.NE.OEFLAG) CLEAR FOUND ;5-3-10
	     END UNTIL (FOUND.GT.0)
	...
	;;;	XCALL INSTR (1,ORDLIN(12,26),KITEM(1,POS),FOUND)
		XCALL INSTR (1,ORDLIN(12,56),KITEM(1,POS),FOUND)
	     	if (found) 
		  begin
		  if (lordno.ne.oordno) READ (24,ORDHDR,lordno) [err=dnfo]
	          IF (OEFLAG.NE.' ' .AND. OLOC.NE.OEFLAG) CLEAR FOUND ;5-3-10
		  end
	        END UNTIL (FOUND.GT.0)
...
;         1         2         3         4         5         6         7         8
	...
	if(oloc .eq. 'O') 
	then	FIND(CHN195,CRHOLD,OORDNO) [ERR=SKIP_ATTR]
	else	goto skip_attr

;;;	FIND(CHN195,CRHOLD,OORDNO) [ERR=SKIP_ATTR]

	PLINE (12,13) = 'H'				;ssq 2-3-10
...
DPXX,
	...
	FOR I FROM 1 THRU 20
		...
		incr xlin		;number of dept's
		PCATAR(I) = S_DEPT
		CLEAR DTSTAMP
		IF (S_STAT)
		THEN	BEGIN
			PCATCOMP(I) = 9
			DTSTAMP(1,5) = S_DATE(5,8), 'ZX/XX'
			DTSTAMP(7,11) = S_TIME,	'XX:XX'
			using s_stat select
			(1),	DTSTAMP(13,13) = 'B'
			(2),	DTSTAMP(13,13) = 'F'
			(3),	DTSTAMP(13,13) = 'C'
		...
		(1),		dline6(24,28) = 'BURN'
		(2),		dline6(24,28) = 'FORM'
		(3),		dline6(24,28) = 'COMPL'
	;;;	(.ge.1),	dline6(24,28) = 'COMPL'
		endusing
...
; 6-6-13 if only 1 completed dept bol info doesn't print
	num_bol = 1	;for heading
	find (28, blhead, oordno) [err=nb_loop]
nb_loop,
	reads (28, blhead, nb_eof)
	if (bhordn .ne. oordno) goto nb_eof
	incr num_bol
	goto nb_loop
nb_eof,

;;;	if (xlin .le. 0) xlin = 1
	if (xlin .lt. num_bol) xlin = num_bol	;6-6-13 for bol's
...
;;;	IF (FYEAR.NE.51)
	...
	PLINE (11,50) = C_WEB
	CALL PRINT
	PLINE(42,57) = ; 9-19-13 ':::CREDIT CODE:'	;SSQ 6-8-04
	PLINE(59,64) = CRDLMT,'XXXXXX'		;SSQ 6-8-04
	PLINE (66,73) = 'SHIP-TO:'		;01-17-10
	PLINE (74,77) = OSHPTO,	'ZZZX'		;01-17-10
	CALL PRINT
	PLINE (4,12) = 'BILL TO: '
	PLINE (13,37) = OCUSNM		;;;
	PLINE (42,50) = 'SHIP TO: '
	...
	PLINE (13,24) = PHONE,	'ZZZ ZZZ ZZZZ'
	CALL PRINT
...
;-
	...
	PLINE (12,23) = XPOL2			;added 8-14-13
		TS_FLAG = 1
		CALL PRINT
		END
;-
...
;;;	WDESC2 = F1_MEMOL(WSEQ12)
	WDESC2(1,90) = F1_MEMOL(WSEQ12)		;ssq 5-23-11
	WDEPT2 = SAVDPT
	  ...
	  BEGIN
	    ...
	    IF (PCATAR(I).NE.'  ') 
	      BEGIN
	        ...
	        IF (PCATCOMP(I).EQ.0) 
		THEN	PLINE (5,17) = ':NO   '
	        ELSE	PLINE (5,17) = PCATSTMP(I)
	      END

;;;	      IF (PCATAR(I+9).NE.'  ') 
	      IF (PCATAR(I+xlin).NE.'  ') 
	      BEGIN
	        PLINE (21,22) = PCATAR(I+xlin)
	        IF (PCATCOMP(I+xlin).EQ.0) 
		THEN	PLINE (23,35) = ':NO   '
	        ELSE	PLINE (23,35) = PCATSTMP(I+xlin)
	      END
...
;;;	FIND(CHN195,CRHOLD,OORDNO) [ERR=NOT_ON_HOLD]
	...
	then	xcall olmsg(w_id,24,'Order has been processed on Credit Card',1)
		...
		xcall w_disp(w_id, wd_pos,1,1, wd_clr, wdc_eow)

		point_size = 20
		sts = %u_wndfont(d_setfont, font_id, face_name, point_size)

	;;;	xcall olmsg(w_id,5,'Order is on Credit Hold!',1)
		xcall w_disp(w_id, wd_pos,7,10, '    ORDER ON HOLD')
	ask_ok,
		xcall w_disp(w_id, wd_pos,10,10, 'Call Dianna @ ext:205')
		xcall w_disp(w_id, wd_pos,12,10, '   OK to continue ')
		xcall w_disp(w_id, wd_reads, a2)
		upcase a2
		if (a2 .ne. 'OK') goto ask_ok
...
ACT2,
	CLEAR W2_ON
;;;	XCALL W_DISP(WND_1,WD_POS,24,1,'.. <cr> NEXT PAGE; <F11> NEW SEARCH; <P>rint <Q> Ship-To <V>iew  <E>mail')
	XCALL W_DISP(WND_1,WD_POS,24,1,'.. <F11> New Search <P>rint <Q> Ship-To <S>tatus <V>iew  <E>mail')
...
; 8-14-13 put this back in per Jess...
...
OPENS,	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	SWITCH = 5
	XCALL filec (cmpcod,75,'SI',075,SWITCH)
	CHN075 = 75
	XCALL filec (cmpcod,3,'SI',182,SWITCH)
	CHN182 = 3
	
	XCALL OECO(CHN182,COINFO,cmpcod)
;;;	XCALL OECO(CHN182,COINFO)

	XCALL filec (cmpcod,4,'SI',193,SWITCH)
	CHN193 = 4
	...
	XCALL filec (cmpcod,6,'SI',195,SWITCH)	;CRHOLD.ISM
	CHN195 = 6

	USING FSELEC SELECT
;;;	USING FYEAR SELECT
	(1), 	BEGIN
		XCALL filec (cmpcod,24,'SI', 44,5)
		XCALL filec (cmpcod,25,'SI', 45,5)
		XCALL filec (cmpcod,26,'SI',175,5)
		XCALL filec (cmpcod,28,'SI',178,5)
		XCALL filec (cmpcod,29,'SI',179,5)
		END
	(2),  BEGIN
		XCALL filec (cmpcod,24,'SI',184,5)
		XCALL filec (cmpcod,25,'SI',185,5)
		XCALL filec (cmpcod,26,'SI',186,5)
		XCALL filec (cmpcod,28,'SI',188,5)
		XCALL filec (cmpcod,29,'SI',189,5)
	      END
	(),  BEGIN
		XCALL FFILc (cmpcod,184,FILON24,SWITCH)
		XCALL FFILc (cmpcod,185,FILON25,SWITCH)
		XCALL FFILc (cmpcod,186,FILON26,SWITCH)
		XCALL FFILc (cmpcod,188,FILON28,SWITCH)
		XCALL FFILc (cmpcod,189,FILON29,SWITCH)
...
;; dptsts POP info...
	...
	WY6 = 2
	POP_WID6(1,5) = "SSPOP"
	POP_WID6(6,8) = WN_TNMBR,	'XXX'
	POP_TITLE6 = "Dept Status"
	RETURN
;-------------------------------------------------------------------
email,	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;	xcall w_proc (wp_create, wnd_2, 'Email',5,77)
;;;	xcall w_proc (wp_place, wnd_2, 4, 0)
;;;	w_id = wnd_2
;;;em_disp,
;;;	clear cngctl
;;;	xcall w_disp (wnd_2, wd_clear)
;;;	xcall w_disp (w_id, wd_pos,1,1, '1. email:   <tab> for list')
;;;	xcall w_disp (w_id, wd_pos,3,1, '2. key: ')
;;;em_mail,
;;;	ctl = '02,01,77,00,AX'	
;;;	call inp
;;;	goto (em_disp, em_end, em_pop),inxctl
;;;	email = entry
;;;ekey,
;;;	ctl = '03,11,09,00,A '
;;;	call inp
...
;;;	ekey = entry(1,9)
;;;
;;;	xcall wancn(w_id, 4, cngctl, whatno)
;;;	goto (em_proc, em_cngbr),cngctl+1
;;;em_cngbr,
;;;	goto (em_mail, ekey), whatno
;;;	goto em_cngbr
;;;em_proc,
;;;	clear coptbl
;;;	tblcod = 'EM'
...
;;;	em_email = email
;;;	switch = 5
;;;	xcall files (19, 'SU', 182, switch)
;;;	if (switch .ne. 9)
;;;		begin
;;;		xcall isio (19, coptbl, tbl_key, store, lokctl)
;;;		close 19
;;;		end
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	clear xnbr
	func = 1	;get
	xcall emcmn (ocusno, xnbr, chn084, func)
...
;-
		w_id = wnd_1
		xcall w_disp (w_id, wd_pos,24,1,wd_clr, wdc_lin, '   Suppress Price Info?')
;;;		xcall w_disp (w_id, wd_pos,4,1, '   Suppress Price Info?')
		xcall w_updt
		ctl = '24,27,01,00,YY'
		call inp
		if (inxctl .eq. 1)
...
;-
		xcall mkest (oordno, email, no_dollars)
		end
;;;	('O'),	if (fyear.eq.51)
	('O'),	if (fselec.eq.1)
		then xcall mkord (oordno, email)
		else xcall mkinv (ordhdr, email, cmpcod)	;5-11-11
	;;;	else xcall mkinv (oordno, email, cmpcod)	;5-11-11
	;;;	else xcall mkinv (oordno, email)
	endusing
...
;;;em_pop,
;;;	call get_em
;;;d_pop,	if (numara .le. 0) goto em_disp
;;;	dline = 'email address'
;;;	xcall pop1 (pop)
...
;;;	;;;(1,4),	begin
;;;	(1),	begin
;;;		dline = parry(pi)	;ssq 3-4-13
;;;		email = dline (11,80)	;ssq 3-4-13
;;;	;;; 3-4-13	email = parry(pi)
;;;		goto em_send
;;;		end
;;;	(4),	begin		;<cr>
;;;		call init_em_ara
...
;;;	tblcod = 'EM'
;;;	em_cust = ocusno
;;;	find (chn182, coptbl, tbl_key) [err=gem_loop]
;;;gem_loop,
;;;	reads (chn182, coptbl, gem_eof, getrfa:tblrfa) [err=gem_eof]
;;;	if (tblcod .ne. 'EM') goto gem_eof
;;;	if (em_cust .ne. ocusno) goto gem_eof
;;;
;;;	incr numara
;;;
;;;	dline(1,9) = em_key
;;;	dline(11,80) = em_email
;;;
;;;	parry(numara) = dline
;;;	parrfa(numara) = tblrfa
;;;
;;;	if (numara .ge. maxara) return	;array is full, but not eof
;;;
;;;	goto gem_loop
;;;
;;;gem_eof,
