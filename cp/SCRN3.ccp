...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	;;;	CTL = '02,19,04,00,#E'
	;;;	DPC=999
		ENTRY(1,4)=DPC,	'ZX.X' [LEFT]
		XCALL W_DISP(W_ID,WD_POS,ROW,COL,ENTRY(1,4))
		XCALL W_UPDT
		END		
		...
		IF (CNGCTL)
			...
			; accessories.
			  ...
			  IF (GAUGE.EQ.16 .AND. STY.NE.3)
				BEGIN
				XCALL OLMSG (WND_1,23,'Must Be "RB3" for 16ga',2)
				GOTO STY
				END
...
TCORN,
;;;	IF(STY.NE.2 .OR. (ACC.NE.1.AND.ACC.NE.4))GOTO BADCNG
...
;;;	IF (ENTRY.EQ.'     ') TGAQTY = X_WGHT / ((DUCLBS * SIZE3) #3)
	IF (ENTRY.EQ.'     ') 
		begin
		cltqty = X_WGHT / ((DUCLBS * SIZE3) #3)
		perm = (sz1+sz2)*2*joint
		perm = (perm*10/12)#1		;to ft
		pqty = (perm*100/50)#2		;50' rolls
		if (perm .gt. (pqty*50)) pqty = pqty + 1	;add a roll
		tgaqty = pqty
		end
	IF (TGAQTY.NE.0)
	THEN	BEGIN
...
;;;	IF(STY.NE.2 .OR. (ACC.NE.1.AND.ACC.NE.4))GOTO BADCNG
	...
	;;;	TCLQTY = ( ( TGAQTY*100 ) / DUCCLE )#1
		TCLQTY = ( ( cltqty*100 ) / DUCCLE )#1	;ssq 4-4-06
...
D_BOTTOM,	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	IF(STY.EQ.1.OR.STY.EQ.5)
		BEGIN
		XCALL W_DISP(W_ID,WD_POS,15,01,'  15. Slips       ')
		XCALL W_DISP(W_ID,WD_POS,16,01,'  16. Drives      ')
		END
	IF(STY.EQ.2.OR.STY.EQ.6)
		BEGIN
		XCALL W_DISP(W_ID,WD_POS,15,01,'  17. TDC Corners ')
