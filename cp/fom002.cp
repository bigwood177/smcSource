...
RECORD	FILLOC
		,A*,	'\\server-dc01\home\Material Data\Pricing\Flat Oval Mapping\Reducers.txt'
...
RECORD	VARS
	KEYCOD	,A5
	KC_R	,A5	;KEY CODE REDUCERS
	KC_E	,A5	;KEY CODE ECCENTRIC REDUCERS
	SAVR	,A15
	SAVE	,A15
	CHN182	,D2
	OPNOK	,D1
...
PROC
	XCALL TERID (V)
	XCALL OUTPT (1,1,2,'FLAT OVAL TABLES - REDUCERS',1)
...
PDISP,
	CNGCTL = 
	XCALL OUTPT (1,1,2,'PRINT FLAT OVAL REDUCERS TABLE',1)
	XCALL OUTPT (4,4,0,'ECC/REDU/BOTH (E/R/B)',1)
ST_DPT,
...
PLOOP,
	...
	USING ERB SELECT
	('E'),	IF (FO_KEY .NE. KC_E) GOTO EOF
	('R'),	BEGIN	
		IF (FO_KEY.LT.KC_R) GOTO PLOOP
		IF (FO_KEY.GT.KC_R) GOTO EOF
		END
	('B'),	IF (FO_KEY .GT. KC_R) GOTO EOF
	ENDUSING

	IF (FO_KEY .NE. TBLCOD) GOTO EOF
...
U_LOOP,
	...
	FOR I FROM 1 THRU 14				;DATA IN FIRST 9 COLUMNS
		BEGIN
...
UPD_TABLE,	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	USING APART SELECT
	('FOER'),	KEYCOD = KC_E	;ECCENTRIC
	('FOR'),	KEYCOD = KC_R	;REDUCER
	ENDUSING

;;;	USING REFO_CODE SELECT
;;;	('RW'),	KEYCOD = KC_R	;REDUCER
;;;	('ERW'),KEYCOD = KC_E	;ECCENTRIC
;;;	ENDUSING
